...
// Reusable constants
const foundryWithNoLoaders = new SecretsFoundry([]);
const foundryWithDummyLoaders = new SecretsFoundry([
...
describe('SecretsFoundry', () => {
  it('Should resolve standard vars', async () => {
    const response = await foundryWithNoLoaders.dotenvExpand(baseRecord);
    expect(response).toStrictEqual(envVars);
  });

  it('Should resolve value for dummy loaders', async () => {
    const response = await Promise.all([
      foundryWithDummyLoaders.resolveVar(
        'sourceA:hello_world',
        envVars
      ),
      foundryWithDummyLoaders.resolveVar(
  ...
  it('Should resolve vars with both simple and dummy foundry vars', async () => {
    const response = await foundryWithDummyLoaders.dotenvExpand(completeBaseRecord);
    expect(response).toStrictEqual({
      ...envVars,
